...
export default function Captcha() {
  ...
  const [recaptchaCheckboxToken, setRecaptchaCheckboxToken] = useSetting('captcha', 'recaptcha_site_token');
  const [recaptchaCheckboxSecret, setRecaptchaCheckboxSecret] = useSetting('captcha', 'recaptcha_secret_token');
  const [recaptchaInvisibleToken, setRecaptchaInvisibleToken] = useSetting('captcha', 'recaptcha_invisible_site_token');
  const [recaptchaInvisibleSecret, setRecaptchaInvisibleSecret] = useSetting('captcha', 'recaptcha_invisible_secret_token');
  const hasBuiltInCaptcha = useSelector('isBuiltInCaptchaSupported')();
  const setErrorFlag = useAction('setErrorFlag');
  const missingRecaptchaCheckboxToken = (type === 'recaptcha' && recaptchaCheckboxToken.trim() === '');
  const missingRecaptchaCheckboxSecret = (type === 'recaptcha' && recaptchaCheckboxSecret.trim() === '');
  const missingRecaptchaInvisibleToken = (type === 'recaptcha-invisible' && recaptchaInvisibleToken.trim() === '');
  const missingRecaptchaInvisibleSecret = (type === 'recaptcha-invisible' && recaptchaInvisibleSecret.trim() === '');
  useEffect(
    () => {
      setErrorFlag(
        missingRecaptchaCheckboxToken || missingRecaptchaCheckboxSecret
        || missingRecaptchaInvisibleSecret || missingRecaptchaInvisibleToken,
      );
    },
    [
